#!/usr/bin/env python3
"""Architecture D+v2.2: Sonnet-Anchored with Balanced Discrimination.

Changes from D+v2.1:
  1. BALANCED SONNET PROMPT: Neither "default approve" (v2, 65% FP) nor "default reject"
     (v2.1, 28 false rejections). Uses category-based criteria instead.
     Key insight: v2.1 rejected valid terms like "resources", "owners", "kind", "limits"
     that are legitimate standalone technical concepts in this domain.
  2. FIXED DEDUP: Don't merge terms with different qualifiers (v1 vs v2, labels vs selectors).
     v2.1 incorrectly merged "cgroup v1" into "cgroup v2", and "selectors" into "labels".
  3. STRUCTURAL FILTER: Kept from v2.1 (works well, 16 removed, 0 false removals).

Key metrics context:
  D+v2:   P=67.1%, R=94.2%, H=32.9%, F1=0.776
  D+v2.1: P=90.6%, R=77.1%, H=9.4%,  F1=0.826
  Target: P≥90%+, R≥85%+, H<10%

Pipeline:
  Phase 1: 3 parallel extractors (Sonnet exhaustive + Haiku exhaustive + Haiku simple)
  Phase 2: Span grounding (deterministic)
  Phase 2.5: Structural term filter
  Phase 3: Vote-based routing (3-vote auto-keep, 2-vote auto-keep, 1-vote → Phase 4)
  Phase 4: Sonnet BALANCED discrimination for 1-vote terms
  Phase 5: Smart assembly + conservative variant dedup

Usage:
    python test_dplus_v2_2.py [--chunks N]
"""
...
RESULTS_PATH = ARTIFACTS_DIR / "dplus_v2_2_results.json"
AUDIT_PATH = ARTIFACTS_DIR / "dplus_v2_2_audit.json"
...
# Key change: Balanced discrimination prompt (neither default approve nor default reject)
SONNET_BALANCED_PROMPT = """You are reviewing candidate technical terms extracted from documentation. Each term was found by only ONE extractor and needs your judgment.
...
def is_variant_of(shorter: str, longer: str) -> bool:
    ...
    CONSERVATIVE: Only merges exact duplicates and strict singular/plural of the SAME base.
    Does NOT merge substring matches — those cause false merges like:
      - "cgroup v1" absorbed by "cgroup v2" (different concepts!)
      - "selectors" absorbed by "labels and selectors" (independent concept!)
      - "kubectl" absorbed by "kubectl get events" (independent concept!)
    """
    ...
    # Singular/plural ONLY for exact base match (no other word differences)
    s_base = s.rstrip("s") if s.endswith("s") and len(s) > 3 else s
    l_base = l.rstrip("s") if l.endswith("s") and len(l) > 3 else l
...
def run_dplus_v2_2_chunk(
    ...
    if needs_review:
        logger.subsection("Phase 4: Sonnet Balanced Discrimination")
        logger.info(f"  Reviewing {len(needs_review)} 1-vote terms (BALANCED mode)...")

        terms_for_review = json.dumps(needs_review, indent=2)
        approval_prompt = SONNET_BALANCED_PROMPT.format(
...
def run_experiment(num_chunks: int = 15):
    ...
    logger = BenchmarkLogger(
        log_dir=LOG_DIR,
        log_file=f"dplus_v2_2_{timestamp}.log",
        console=True,
        min_level="INFO",
    )

    logger.section("D+v2.2: Sonnet-Anchored with Balanced Discrimination")
    logger.info(f"GT: {GT_V2_PATH}")
    logger.info(f"Chunks: {num_chunks}")
    logger.info(f"Timestamp: {timestamp}")
    logger.info("Changes from D+v2.1:")
    logger.info("  1. Balanced Sonnet prompt (neither default APPROVE nor REJECT)")
    logger.info("  2. Conservative dedup (no substring/component merging)")
    logger.info("  3. Structural term filter (kept from v2.1)")
    ...
    for i, chunk in enumerate(chunks):
        ...
        start = time.time()
        final_terms, audit_trail = run_dplus_v2_2_chunk(content, chunk_id, logger)
    ...
    logger.subsection("Comparison to Previous Versions")
    logger.info(f"                 D+v2      D+v2.1    D+v2.2    v2→v2.2")
    logger.info(f"  Precision:     67.1%     90.6%     {agg['precision']:.1%}     {agg['precision'] - 0.671:+.1%}")
    logger.info(f"  Recall:        94.2%     77.1%     {agg['recall']:.1%}     {agg['recall'] - 0.942:+.1%}")
    logger.info(f"  Hallucination: 32.9%     9.4%      {agg['hallucination']:.1%}     {agg['hallucination'] - 0.329:+.1%}")
    logger.info(f"  F1:            0.776     0.826     {agg['f1']:.3f}     {agg['f1'] - 0.776:+.3f}")
    ...
    results = {
        ...
        "timestamp": timestamp,
        "changes_from_v2_1": [
            "Balanced Sonnet prompt (category-based, neither default approve nor reject)",
            "Conservative dedup (singular/plural only, no substring merging)",
            "Structural term filter (kept from v2.1)",
        ],
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="D+v2.2 benchmark")
    parser.add_argument(
        "--chunks",
        type=int,
        default=15,
