"""Enriched Hybrid retrieval with LLM query rewriting - BM25 + semantic + query rewrite."""
...
class EnrichedHybridLLMRetrieval(RetrievalStrategy, EmbedderMixin):
    def __init__(
        self,
        name: str = "enriched_hybrid_llm",
        rrf_k: int = 60,
    ...
    def _trace_log(self, msg: str):
        if self.debug:
            ...
            get_logger().trace(f"[enriched-hybrid-llm] {msg}")
    ...
    def retrieve(self, query: str, k: int = 5) -> list[Chunk]:
        ...
        # Then expand the (possibly rewritten) query
        expanded_query = expand_query(rewritten_query, debug=self.debug)
