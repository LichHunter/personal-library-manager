...
- **Informed benchmark** (80%): Same 25 documents but questions use proper Kubernetes terminology (e.g., "SubjectAccessReview", "kube-proxy", "ABAC"). Tests: "Does technical vocabulary improve retrieval?" - **YES** (+44% improvement proves vocabulary gap is the primary retrieval challenge).
...
**Key Difference**: No LLM-based query rewriting. Uses original user query directly, skipping the Claude Haiku transformation step. This reduces latency by eliminating the LLM call (~200-500ms per query) but may slightly reduce accuracy for complex queries that benefit from semantic rewriting.
