"""Modular Enriched Hybrid LLM retrieval orchestrator.
...
The orchestrator produces 100% IDENTICAL results to the original implementation,
...
class ModularEnrichedHybridLLM:
    ...
    This class replicates the EXACT behavior of EnrichedHybridLLMRetrieval from
    ...
    - Uses QueryRewriter for LLM-based query rewriting
    ...
    Attributes:
        ...
        rewrite_timeout: Timeout for LLM query rewriting (default 5.0s)
    ...
    def __init__(
        ...
        """Initialize ModularEnrichedHybridLLM orchestrator.
        ...
        self._query_rewriter = QueryRewriter(timeout=rewrite_timeout)
    ...
    def _trace_log(self, msg: str):
        ...
        if self.debug:
            print(f"[modular-enriched-hybrid-llm] {msg}", flush=True)
    ...
    def retrieve(self, query: str, k: int = 5) -> list[Any]:
        ...
        2. Expand (possibly rewritten) query with domain terms
        3. Select adaptive weights based on expansion_triggered
        4. Get semantic and BM25 scores
        5. Fuse with RRF (semantic FIRST, BM25 SECOND)
        ...
        # Rewrite query with LLM (lines 188-202)
        rewrite_start = time.time()
        try:
            query_obj = Query(text=query)
            rewritten_result = self._query_rewriter.process(query_obj)
            rewritten_query = rewritten_result.rewritten
        except Exception:
        ...
        # Expand the (possibly rewritten) query (lines 204-208)
        # Use direct expansion to match EXACT behavior
        expanded_query, expansion_triggered = self._expand_query_direct(rewritten_query)
