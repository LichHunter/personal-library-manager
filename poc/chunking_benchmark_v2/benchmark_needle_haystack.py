...
def run_benchmark():
    ...
    # Load questions
    if not NEEDLE_QUESTIONS_FILE.exists():
        raise FileNotFoundError(
            f"Questions file not found: {NEEDLE_QUESTIONS_FILE}\n"
            "Run --generate-questions first."
        )
    
    with open(NEEDLE_QUESTIONS_FILE) as f:
    ...
    # Save results
    RETRIEVAL_RESULTS_FILE.parent.mkdir(parents=True, exist_ok=True)
    ...
    print(f"Needle found rate: {found_count}/{len(questions)} ({output['summary']['needle_found_rate']}%)")
    print(f"Average latency: {avg_latency:.0f}ms")
    print(f"\nResults saved to: {RETRIEVAL_RESULTS_FILE}")
