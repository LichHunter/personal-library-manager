...
class FixedSizeStrategy(ChunkingStrategy):
    ...
    def name(self) -> str:
        overlap_pct = (
            int(self.overlap / self.chunk_size * 100) if self.chunk_size else 0
        )
    ...
    def chunk(self, document: Document) -> list[Chunk]:
        ...
        logger = get_logger()
        logger.debug(
            f"Chunking document '{document.id}': {len(words)} words, {len(text)} chars"
        )
        ...
        while start_word < len(words):
            ...
            if self.respect_sentences and end_word < len(words):
                ...
                for i in range(
                    end_word,
                    max(start_word + self.words_per_chunk // 2, start_word),
                    -1,
                ):
            ...
            # Debug logging for chunk boundaries
            word_count = end_word - start_word
            token_estimate = int(word_count * 1.33)
            content_preview = chunk_text[:100].replace("\n", " ")
            logger.debug(
                f"  Chunk {chunk_idx}: chars [{start_char}-{end_char}] "
                f"words={word_count} tokens≈{token_estimate} "
