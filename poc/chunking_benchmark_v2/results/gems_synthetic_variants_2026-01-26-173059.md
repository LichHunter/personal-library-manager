...
**Date**: 2026-01-26T17:30:59.919211
**Queries Tested**: 15
...
5. [doc_id: deployment_guide, chunk_id: deployment_guide_fix_3, score: 0.200]
00029|    > Deploy PostgreSQL using the Bitnami Helm chart: ```yaml # postgres-values.yaml global: postgresql: auth: username: cloudflow database: cloudflow existingSecret: postgres-credentials image: tag: "14.10.0" primary: resources: limits: cpu: 4000m memory: 8Gi requests: cpu: 2000m memory: 4Gi persistence: enabled: true size: 100Gi storageClass: gp3 extendedConfiguration: | max_connections = 100 shared_buffers = 2GB effective_cache_size = 6GB maintenance_work_mem = 512MB checkpoint_completion_target = ...
00030| 
00031| **Baseline Score**: 5/10
**New Score**: 8/10
**Notes**: Chunk 1 directly answers with PgBouncer config (pool_mode=transaction) and read replicas solution. Excellent match to expected answer. Minor: doesn't explicitly mention max_db_connections=100 in visible excerpt.
...
**Baseline Score**: 6/10
**New Score**: 7/10
**Notes**: Chunk 1 has HPA config with minReplicas, maxReplicas, targetCPU visible. Chunk 5 mentions API Gateway 2 vCPU, 4GB RAM. Good coverage but chunks are fragmented - need to piece together the relationship between HPA and resource requirements.
**Type**: temporal
...
**Baseline Score**: 7/10
**New Score**: 8/10
**Notes**: Chunk 2 directly addresses timeout sequence with solutions (increase timeout to 7200s). Chunk 1 mentions error notifications. Covers the expected answer well - automatic termination at 3600s and Enterprise custom timeout option visible.
**Type**: temporal
**Root Causes**: EMBEDDING_BLIND
...
**Baseline Score**: 4/10
**New Score**: 3/10
**Notes**: Chunks discuss caching, query optimization, and version control but don't address cache propagation timeline or Redis TTL. Expected answer mentions 1-hour TTL and 94.2% cache hit rate - not found in retrieved chunks. Poor retrieval quality.
**Type**: temporal
**Root Causes**: EMBEDDING_BLIND
...
2. [doc_id: architecture_overview, chunk_id: architecture_overview_fix_12, score: 0.500]
...
**Baseline Score**: 7/10
**New Score**: 9/10
**Notes**: Chunk 1 directly answers with automatic failover timelines: Database 30-60s, Redis <10s, Kafka <30s. Perfect match to expected answer. Excellent retrieval quality.
**Type**: comparative
**Root Causes**: VOCABULARY_MISMATCH, EMBEDDING_BLIND
...
**Baseline Score**: 2/10
**New Score**: 6/10
**Notes**: Chunk 1 has PgBouncer config with pool_mode=transaction. Chunk 2 shows max_connections=100. Covers PgBouncer side well but missing direct comparison to "direct PostgreSQL connections" and the 1000 client vs 100 DB connections distinction. Partial answer.
**Type**: comparative
**Root Causes**: EMBEDDING_BLIND
...
5. [doc_id: architecture_overview, chunk_id: architecture_overview_fix_8, score: 0.200]
...
**Baseline Score**: 6/10
**New Score**: 7/10
**Notes**: Chunk 2 shows exponential backoff (2^n seconds) and max 3 retries. Chunk 3 mentions retry policies. Missing explicit comparison of fixed vs linear vs exponential strategies. Covers exponential well but incomplete comparative analysis.
**Type**: comparative
**Root Causes**: EMBEDDING_BLIND
...
2. [doc_id: deployment_guide, chunk_id: deployment_guide_fix_2, score: 0.500]
...
**Baseline Score**: 5/10
**New Score**: 8/10
**Notes**: Chunk 2 shows livenessProbe (/health) and readinessProbe (/ready) with different initialDelaySeconds and periodSeconds. Excellent match to expected answer distinguishing liveness vs readiness checks. Clear and direct.
**Type**: negation
...
5. [doc_id: user_guide, chunk_id: user_guide_fix_7, score: 0.200]
...
**Baseline Score**: 6/10
**New Score**: 7/10
**Notes**: Chunk 1 has rate limit headers (X-RateLimit-Limit, X-RateLimit-Remaining, X-RateLimit-Reset). Chunk 3 shows rate limit checking code. Covers what NOT to do (keep hammering) implicitly through proper handling examples. Good but could be more explicit about the "don't" part.
**Type**: negation
**Root Causes**: NEGATION_BLIND
...
**Baseline Score**: 7/10
**New Score**: 4/10
**Notes**: Chunks discuss JWT validation, clock skew, and authentication but don't address HS256 vs RS256 algorithm difference. Expected answer requires understanding asymmetric (RS256) vs symmetric (HS256) - not found in retrieved chunks. Poor match.
**Type**: negation
**Root Causes**: NEGATION_BLIND
...
**Baseline Score**: 7/10
**New Score**: 5/10
**Notes**: Chunks mention schedule calculator and retry policies but don't address minimum scheduling interval constraint. Expected answer requires knowledge of "minimum 1 minute" restriction - not found. Chunks discuss scheduling but miss the key constraint.
**Type**: negation
**Root Causes**: NEGATION_BLIND
...
4. [doc_id: api_reference, chunk_id: api_reference_fix_0, score: 0.250]
   > # CloudFlow API Reference Version 2.1.0 | Last Updated: January 2026 ## Overview The CloudFlow API is a RESTful service that enables developers to programmatically manage cloud workflows, data pipelines, and automation tasks. This documentation provides comprehensive details on authentication, endpoints, request/response formats, error handling, and best practices. **Base URL:** `https://api.cloudflow.io/v2` **API Status:** https://status.cloudflow.io ## Authentication CloudFlow supports three a...
...
**Baseline Score**: 6/10
**New Score**: 6/10
**Notes**: Chunk 1 mentions token expiry (3600 seconds) and refresh token validity (7-30 days). Chunk 2 discusses cache TTL strategies. Covers token expiration but missing explicit explanation of what happens without refresh logic. Partial answer.
**Type**: negation
**Root Causes**: NEGATION_BLIND
...
**Baseline Score**: 5/10
**New Score**: 7/10
**Notes**: Chunk 1 directly addresses hardcoding API keys as security risk. Chunk 3 mentions Vault integration and secret rotation. Covers the "why not" well but missing explicit reference to {{secrets.API_TOKEN}} syntax and Settings > Secrets location.
**Type**: implicit
**Root Causes**: VOCABULARY_MISMATCH, EMBEDDING_BLIND
...
**Baseline Score**: 6/10
**New Score**: 7/10
**Notes**: Chunk 1 shows EXPLAIN ANALYZE for slow queries and index checking. Chunk 2 mentions bottleneck analysis. Covers debugging approach well but missing the specific latency breakdown percentages (Auth 18%, DB 64%, etc.) and cloudflow metrics latency-report command.
**Type**: implicit
**Root Causes**: VOCABULARY_MISMATCH, EMBEDDING_BLIND
...
**Baseline Score**: 7/10
**New Score**: 8/10
**Notes**: Chunk 1 directly addresses long-running data processing with timeout increase solution (7200s). Chunk 2 mentions parallel processing optimization. Covers main solutions well but missing checkpointing (every 300s) and workflow splitting strategies.
