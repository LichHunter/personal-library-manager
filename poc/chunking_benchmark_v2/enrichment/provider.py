...
class OllamaProvider(LLMProvider):
    ...
    def generate(
        self, prompt: str, model: str = "llama3.2:3b", timeout: int = 90
    ) -> str:
        log = get_logger()
        log.debug(f"[ollama] model={model} prompt_len={len(prompt)} timeout={timeout}s")
        log.debug(
            f"[ollama] PROMPT: {prompt[:500]}{'...' if len(prompt) > 500 else ''}"
        )
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                log.debug(
                    f"[ollama] SUCCESS in {elapsed:.2f}s response_len={len(response)}"
                )
                log.debug(
                    f"[ollama] RESPONSE: {response[:500]}{'...' if len(response) > 500 else ''}"
                )
                return response
            else:
                log.warn(
                    f"[ollama] FAILED returncode={result.returncode} stderr={result.stderr[:200]}"
                )
                return ""
        except subprocess.TimeoutExpired:
            elapsed = time.time() - start_time
            log.warn(f"[ollama] TIMEOUT after {elapsed:.2f}s (limit={timeout}s)")
            return ""
        except Exception as e:
            elapsed = time.time() - start_time
            log.error(f"[ollama] ERROR after {elapsed:.2f}s: {e}")
            return ""
...
class AnthropicProvider(LLMProvider):
    ...
    def generate(
        ...
        try:
            ...
            elapsed = time.time() - start_time
            log.warn(f"[anthropic] TIMEOUT after {elapsed:.2f}s (limit={timeout}s)")
            return ""
        except Exception as e:
            elapsed = time.time() - start_time
...
def get_provider(model: str) -> LLMProvider:
    ...
    if any(x in model_lower for x in ["claude", "haiku", "sonnet"]):
